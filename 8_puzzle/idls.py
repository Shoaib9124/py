...
def idls(start, goal, x, y, out, max_depth=20):
    for depth in range(max_depth + 1):
        out.write(f"\nTrying depth limit = {depth}\n")
        if dls(start, goal, x, y, out, limit=depth):
            out.write(f"Goal found at depth {depth}\n")
            return True
    return False
...
if __name__ == "__main__":
    ...
    with open("output_idls.txt", "w") as out:
        found = idls(start, goal, x, y, out, limit)
        if not found:
