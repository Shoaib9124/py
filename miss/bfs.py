...
def is_valid(m_left, c_left):
    m_right = 3 - m_left
    c_right = 3 - c_left
    # Valid if missionaries are not outnumbered on either side
    return (m_left == 0 or m_left >= c_left) and (m_right == 0 or m_right >= c_right)

def bfs(start, goal, out):
    q = deque([(start, [start])])
    vis = set([start])

    moves = [(2,0),(0,2),(1,0),(0,1),(1,1)]  # Possible boat moves

    while q:
        (m_left, c_left, boat), path = q.popleft()
        ...
        for m, c in moves:
            ...
            if 0 <= new_state[0] <= 3 and 0 <= new_state[1] <= 3:
                if is_valid(new_state[0], new_state[1]) and new_state not in vis:
                    vis.add(new_state)
...
if __name__ == "__main__":
    ...
    with open("output_bfs.txt", "w") as out:
        out.write("Missionaries and Cannibals BFS Solution:\n")
        bfs(start, goal, out)
